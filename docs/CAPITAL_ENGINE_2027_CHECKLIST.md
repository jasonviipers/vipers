...
- [~] Use point-in-time market/news/fundamental data. The typed PIT contract exists (`src/ai/capital-engine/point-in-time.ts`): a dataset is an append-only step function of observations (value, `asOf`, `validTo` retraction, source); `pitValueAt` is a binary-search as-of reader that refuses — typed `PitLeakageError`, never a silent nearest-match — queries before the first stamp or into a retracted gap; `createPitDataset` rejects unsorted/duplicate stamps and impossible retractions; `pitBarsFromSimulationBars`/`pitBarAt` bridge the simulator's bar model (same never-newer-than rule its `submittedAt` lookup follows); `hashPitDataset` pins the exact consumed dataset into a promotion record for reproducibility. Tested: as-of semantics, mid-bar decision moments, leakage refusal, retraction gaps, hash identity (`test/ai/capital-engine/point-in-time.test.ts`). Ingestion now WIRED: `src/lib/jobs/pit-ingestion-job.ts` pulls paginated historical bars from the Alpaca feed (`getHistoricalBars`, the only wired historical source), converts them to PIT points (asOf = bar open time), validates them through the contract, and stores them durably in `pit_datasets` (migration `0014`, unique per asset/broker/window) with `datasetHash` pinned for promotion-record reproducibility; idempotent per window, refuses thin windows (< 24 bars), never edits stored datasets. Triggered via `POST /api/jobs/pit-ingestion` (write-access). The news/sentiment axis is now WIRED into the same contract: `pitSentimentFromObservations` (pure adapter) turns the durable scraper archive (`scraped_messages`) into a sentiment PIT dataset whose as-of stamp is each message's `fetchedAt` — the OBSERVATION time, never `postedAt` (a post enters our world only when the scraper fetched it) — with values recomputed by VADER exactly like the live `fetchMarketSignals` tool and aggregated over the trailing 7-day window (count-weighted, unit scale); `runPitSentimentIngestion` persists it (`pit_datasets.kind = "sentiment"`, migration `0015`, brokerId `scraper-archive`), never edits stored rows, and refuses an empty archive (nothing fabricated). A backtest RUNNER now exists: `src/ai/capital-engine/backtest-runner.ts` (pure) walks a decision grid over the ingested datasets — every bar/sentiment read via the leakage-refusing `pitValueAt`, a deterministic SMA(3/12)-momentum/sentiment-gate reference strategy, fills through `simulateOrder` (fees/spread/slippage/latency/participation), end-of-data liquidation at the last latency-actionable stamp, and the accounting identity `endEquity = capital + Σ realized pnl`; `POST /api/jobs/backtest` (write-access) loads the stored datasets, VERIFIES each against its pinned `datasetHash` (409 on corruption — never runs on unverified bytes), runs the runner, and returns metrics + both dataset hashes for promotion-record pinning. Tested in `test/ai/capital-engine/backtest-runner.test.ts` + extended PIT suite (as-of/leakage discipline, never-newer-than sentiment reads, accounting integrity, warm-up/gate semantics); E2E verified against a live server with every path failing closed (404 without ingestion, honest refusal on OKX-active/no-creds/empty-archive). FIRST RUNS OVER REAL DATA COMPLETE (2026-09-19): 30 days of real Alpaca BTC/USD hourly bars (721 bars, pinned `datasetHash 87aea4c5…`) ingested after switching the active broker to `alpaca`; `POST /api/jobs/backtest` ran end to end over them (720 decisions, 76 trades) and `POST /api/jobs/walk-forward` ran 4 folds (pooled winner SMA 4/16, one cold holdout replay, ~flat holdout equity) with the dataset-identity chain intact. The run surfaced and fixed two latent integration bugs: the Alpaca crypto historical-bars call (locale-scoped path + `symbols` query param + per-symbol-keyed response) and a pit-store reader that expected a nested envelope never written (now reads the flat written shape). REMAINS PENDING: the sentiment axis (needs live scrape history), a wider ingested window, and operator review of the holdout report.
...
- [x] Add deterministic replay/simulation fixtures and a reusable replay contract. (Historical dataset ingestion is now wired — bars + sentiment into `pit_datasets` with pinned hashes — and the backtest runner consumes them; a first run over real ingested data remains pending on credentials.)
- [~] Run walk-forward and untouched holdout evaluations. The evaluation core now exists: `src/ai/capital-engine/walk-forward.ts` (pure, db-free — the caller supplies verified datasets) with a PREDECLARED layout — the holdout is the most recent max(30, 20% of N) bars and is structurally separated from selection (every leg slice is an INDEX cut out of the eval region, so no selection run can even fill from a holdout bar); the eval region is walked forward in `folds` steps (fold f scores every grid candidate IN-SAMPLE on eval[f·S…f·S+W), then replays the fold winner COLD — fresh strategy instance, no indicator carry-over — on the following step, so every eval bar after the first IS window serves as out-of-sample data exactly once); sentiment is pre-sliced per span (trailing warmup kept, future stamps REMOVED). The searched grid is the published `REFERENCE_SMA_GRID` (backtest-runner.ts) — fixed at publication; widening it after seeing results would be data snooping, so a different grid is a NEW evaluation. The pooled winner (most fold spans won; ties by mean IS equity, then grid order) is replayed on the holdout slice ONCE, cold, at the end; every slice and the full datasets are pinned by canonical hash for the promotion record. `POST /api/jobs/walk-forward` (write-access) consumes datasets ONLY through the pit-store verified loaders (404/409/500 fail-closed) and additionally asserts the run's pinned dataset hashes equal the store-verified hashes before reporting (identity-chain check). Tested in `test/ai/capital-engine/walk-forward.test.ts` (layout cuts and tiling without gaps/overlaps, OOS-after-IS discipline, IS-best selection, aggregate pooling, slice-hash pinning, determinism, refusals, and the published grid driving a full run). REMAINS PENDING: operator review of the holdout report (the first real-data run is complete — 2026-09-19, see the PIT item above) and re-runs over a wider ingested window.
...
- [~] Run paper mode against the intended venue/account behavior. The venue contract is now verifiable: `src/ai/capital-engine/paper-venue-verify.ts` names the two paper paths — OKX demo → the LOCAL notional paper book (no venue behavior exists there) vs Alpaca demo → the Alpaca PAPER API, which IS the intended venue (own accounts, order lifecycle, same REST contract as live). `POST /api/strategies/paper-venue-verify` (write-access) probes it READ-ONLY and fails closed per check: paper-route-active (the active slot must resolve to the paper endpoint family — a live slot never passes), account-readable (authenticated equity read), order-lifecycle-readable (client_order_id query round-trips via the new non-throwing `findOrderByClientOrderId`, so paper submissions are reconcilable), reconciliation-surface-parity (positions read matches what the live reconciliation job consumes). Evaluator pure and unit-tested fail-closed (`test/ai/capital-engine/paper-venue-verify.test.ts`); 503 with per-check details when any check fails. REMAINS PENDING: the actual paper exercise window — running the pipeline end to end against the venue (orders through the normal path) and reviewing fills/reconciliations before canary capital (§10's "emergency stop exercised in paper mode" likewise remains).
...
- [x] Focused unit tests pass for proposal parsing, intent hashing, risk kernel, ledger balancing, promotion, plugin contracts, plugin runtime, and execution-mode separation. (Full reconciliation integration coverage remains pending; latest full-suite run: 269 passed, 0 failed.)
