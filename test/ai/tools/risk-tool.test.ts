...
describe("evaluateProposalRisk", () => {
  ...
  it("allows a proven protective reduction while new risk is halted", () => {
    const result = evaluateProposalRisk(
      {
        asset: "BTC-USD",
        confidence: 0,
        proposedPositionPct: 100,
        reducesRisk: true,
        riskType: "PROTECTIVE_REDUCTION",
      },
      BASE_LIMITS,
      ctx({ killSwitchEnabled: true }),
    );
    expect(result.approved).toBe(true);
    expect(result.reason).toContain("Protective reduction");
  });

  it("rejects an unproven protective reduction", () => {
    const result = evaluateProposalRisk(
      {
        asset: "BTC-USD",
        confidence: 0,
  ...
  it("rejects a proposal that exceeds leverage", () => {
    const result = evaluateProposalRisk(
      { asset: "BTC-USD", confidence: 0.9, proposedLeverage: 4 },
      { ...BASE_LIMITS, maxLeverage: 3 },
      ctx(),
    );
    expect(result.reason).toContain("Leverage");
  });

  it("rejects concentration, correlation, volatility, spread, and drawdown breaches", () => {
    const checks = [
      [
        { currentConcentrationPct: 48 },
        { maxConcentrationPct: 50 },
        "Concentration",
      ],
      [{ currentCorrelation: 0.9 }, { maxCorrelation: 0.8 }, "Correlation"],
      [{ currentVolatilityPct: 12 }, { maxVolatilityPct: 10 }, "Volatility"],
      [{ spreadBps: 40 }, { maxSpreadBps: 25 }, "Spread"],
      [{ currentDrawdownPct: 20 }, { maxDrawdownPct: 15 }, "Drawdown"],
    ] as const;
    for (const [context, limits, reason] of checks) {
      const result = evaluateProposalRisk(
        { asset: "BTC-USD", confidence: 0.9, proposedPositionPct: 5 },
        { ...BASE_LIMITS, ...limits },
        ctx(context),
      );
      expect(result.approved).toBe(false);
